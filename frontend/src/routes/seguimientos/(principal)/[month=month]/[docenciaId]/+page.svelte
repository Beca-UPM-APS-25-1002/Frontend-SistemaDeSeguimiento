<script lang="ts">
  ...
  let selectedTema: UnidadDeTrabajo | undefined = $derived(
    data.unidadesDeTrabajo.length > 0
...
{#if data.unidadesDeTrabajo.length === 0}
  ...
  <h1 class="text-lg alert alert-error mt-4">
    ...
    No se han encontrado unidades de trabajo. Contacta con tu administrador para
    ...
    {#if data.seguimientoActual && data.seguimientoActual?.profesor.id != data.user?.id}
      <div transition:slide>
        <p class="text-sm italic">
          Este seguimiento fue realizado originalmente por {data
            .seguimientoActual?.profesor.nombre}
        </p>
      </div>
    {/if}
  </div>
  ...
  >
    ...
    <div class="w-full flex flex-wrap transition-all">
      ...
      <div class="w-full md:w-[49%] mb-4 md:mb-0 transition-all">
        <fieldset
          class="fieldset border border-base-300 p-4 rounded-box h-full"
        >
          <!--Checkboxes to select completed units of work-->
          <legend class="fieldset-legend"
            >Situación actual de la docencia</legend
          >
          <legend class="fieldset-legend"
          ...
          >
            <option disabled selected value=""
              >Selecciona la unidad actual</option
            >
            {#each data.unidadesDeTrabajo as tema}
          ...
          >
            <option disabled selected value=""
              >Elige la evaluación actual</option
            >
      ...
      <div class="w-full md:w-[49%] flex flex-col">
        ...
        <fieldset class="fieldset border border-base-300 p-4 rounded-box">
          <legend class="fieldset-legend"
            >Cumplimiento de la programación</legend
          >
          ...
          {#if !cumple_programacionValue}
            <div transition:fly={{ y: -20, duration: 300 }}>
              ...
              <p class="alert alert-outline alert-warning">
                ...
                Se recuerda que una vez aprobada la programación por el claustro
                no se pueden realizar cambios en la programación si no están debidamente
                justificados.
              </p>
              <legend class="fieldset-legend">
